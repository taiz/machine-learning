...
class MyChain(chainer.Chain):
    def __init__(self):
        ...
        with self.init_scope():
            self.conv1=L.Convolution2D(1, 16, 3, 1, 1) # 1層目の畳み込み層（チャンネル数は16）
            self.conv2=L.Convolution2D(16, 64, 3, 1, 1) # 2層目の畳み込み層（チャンネル数は64）
...
data_train = data_train.reshape((len(data_train), 1, 8, 8))#1x8x8の行列に変更
...
chainer.backends.cuda.get_device_from_id(0).use() # GPU利用を宣言．0番のGPUを利用（1台の場合は0）
model.to_gpu() # GPUメモリ上にモデルをコピーoptimizer = chainer.optimizers.Adam()
...
trainer.extend(extensions.Evaluator(test_iter, model, device=0))# エポック数の表示
